...
import { BookOpen, ArrowRight, ArrowLeft, Check, RotateCcw, ChevronLeft, ChevronRight } from 'lucide-react';

const MOCK_PAGES = [
    { type: 'cover', label: 'עטיפה', bg: 'from-amber-950 to-stone-900' },
    { type: 'full_bleed', label: 'עמוד 1', bg: 'from-stone-800 to-amber-900' },
    { type: 'spread_2', label: 'עמודים 2–3', bg: 'from-amber-900 to-stone-800' },
    { type: 'grid_4', label: 'עמודים 4–5', bg: 'from-stone-700 to-amber-800' },
    { type: 'text_quote', label: 'עמוד 6', bg: 'from-amber-800 to-stone-700' },
    { type: 'spread_2', label: 'עמודים 7–8', bg: 'from-stone-800 to-amber-900' },
    { type: 'grid_4', label: 'עמודים 9–10', bg: 'from-amber-900 to-stone-800' },
    { type: 'back', label: 'עטיפה אחורית', bg: 'from-stone-900 to-amber-950' },
];
...
function PreviewPageContent() {
    ...
    return (
        <div dir="rtl" style={{ background: 'var(--dark)', minHeight: '100vh' }}>
            <header className="px-3 sm:px-6 py-3 sm:py-4" style={{ borderBottom: '1px solid rgba(255,0,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(26,18,8,0.95)', backdropFilter: 'blur(12px)' }}>
                <Link href={`/order/customize?event=${event}&photos=${photos}`} style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none', flexShrink: 0 }}>
                    <ArrowRight size={16} style={{ color: 'rgba(255,255,255,0.4)' }} className="hidden sm:block" />
                    <div className="w-7 h-7 rounded-full brand-gradient flex items-center justify-center">
                        <BookOpen size={13} className="text-white" />
                    </div>
                    <span className="font-bold hidden sm:inline" style={{ fontFamily: 'Heebo, sans-serif', color: 'white' }}>Holialby</span>
                </Link>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
                <div className="text-center mb-6 sm:mb-8">
                    <div className="section-tag" style={{ display: 'inline-flex', marginBottom: 12, background: 'rgba(255,0,0,0.15)', color: 'var(--primary-light)' }}>שלב 4 מתוך 5 · תצוגה מקדימה מונחית AI</div>
                    <h1 className="text-2xl sm:text-3xl font-bold mb-2 text-white" style={{ fontFamily: 'Heebo, sans-serif' }}>
                        תצוגה מקדימה של האלבום
                    </h1>
                    <p className="text-xs sm:text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
                        {names} · {photos} תמונות · ערכת נושא {themeNameMap[theme] || theme}
                    </p>
                </div>
                    ...
                    <div className="lg:col-span-2">
                            ...
                            {page.type === 'cover' && (
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-4 sm:p-8 text-center">
                                    ...
                                    <p style={{ color: 'var(--primary-light)', letterSpacing: 3, fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }} className="text-[10px] sm:text-xs">
                                        {event === 'wedding' ? 'אלבום חתונה' : 'אלבום ' + (eventNameMap[event] || event)}
                                    </p>
                                    <h2 className="text-xl sm:text-[32px]" style={{ fontFamily: 'Heebo, sans-serif', color: 'white', fontWeight: 700, marginBottom: 8 }}>{names}</h2>
                                    <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>יוני 2025</p>
                            ...
                            {page.type === 'full_bleed' && (
                                <div className="absolute inset-0 flex items-end">
                                    <div className="p-4 sm:p-6 w-full" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.8), transparent)' }}>
                                        <p style={{ color: 'var(--primary-light)', fontSize: 10, letterSpacing: 2, marginBottom: 4 }}>הטקס</p>
                                        <p className="text-base sm:text-xl" style={{ color: 'white', fontFamily: 'Heebo, sans-serif', fontWeight: 600 }}>הרגע שבו הכל השתנה</p>
                                    </div>
                                </div>
                            )}
                            {page.type === 'spread_2' && (
                                <div className="absolute inset-0 grid grid-cols-2 gap-1.5 sm:gap-2 p-2 sm:p-4">
                                    {[0, 1].map(i => (
                                        <div key={i} className="rounded-md sm:rounded-lg" style={{ background: `rgba(255,255,255,${0.05 + i * 0.03})`, display: 'flex', alignItems: 'flex-end', padding: 8 }}>
                                            <p className="hidden sm:block" style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11 }}>כיתוב תמונה {i + 1}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            {page.type === 'text_quote' && (
                                <div className="absolute inset-0 flex items-center justify-center p-5 sm:p-10 text-center">
                                    <div>
                                        ...
                                        <p className="text-sm sm:text-xl" style={{ fontFamily: 'Heebo, sans-serif', color: 'white', lineHeight: 1.7, fontStyle: 'italic', marginBottom: 12 }}>
                                            יהי ביתכם מלא בשמחה, בצחוק ובאהבה אין סופית — מעתה ועד עולם.
                                        </p>
                                        <p style={{ color: 'var(--primary-light)', fontSize: 11, letterSpacing: 1 }}>— ברכה מסורתית</p>
                                    </div>
                                </div>
                            )}
                            {page.type === 'back' && (
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 sm:p-8">
                                    ...
                                    <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>עוצב על ידי Holialby</p>
                                </div>
                            )}

                            {/* סימן מים */}
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                <p className="text-3xl sm:text-5xl" style={{ color: 'rgba(255,255,255,0.04)', fontWeight: 900, transform: 'rotate(-30deg)', userSelect: 'none', whiteSpace: 'nowrap' }}>תצוגה מקדימה</p>
                            </div>

                            {/* תווית עמוד */}
                            <div className="absolute top-2 left-2 sm:top-3 sm:left-3 glass px-2 sm:px-3 py-1 rounded-full" style={{ fontSize: 10, color: 'var(--primary)' }}>
                                {page.label}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                            <button onClick={() => setCurrentPage(Math.min(MOCK_PAGES.length - 1, currentPage + 1))} disabled={currentPage === MOCK_PAGES.length - 1} className="btn-outline flex-shrink-0" style={{ padding: '8px 12px', fontSize: 14, opacity: currentPage === MOCK_PAGES.length - 1 ? 0.4 : 1, borderColor: 'rgba(255,255,255,0.25)', color: 'rgba(255,255,255,0.7)' }}>
                                <ChevronRight size={16} /> <span className="hidden sm:inline">הבא</span>
                            </button>

                            {/* תמונות ממוזערות של עמודים */}
                            ...
                            <button onClick={() => setCurrentPage(Math.max(0, currentPage - 1))} disabled={currentPage === 0} className="btn-outline flex-shrink-0" style={{ padding: '8px 12px', fontSize: 14, opacity: currentPage === 0 ? 0.4 : 1, borderColor: 'rgba(255,255,255,0.25)', color: 'rgba(255,255,255,0.7)' }}>
                                <span className="hidden sm:inline">הקודם</span> <ChevronLeft size={16} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        {!approved ? (
                            <div className="rounded-2xl p-4 sm:p-6" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,0,0,0.2)' }}>
                                <h3 className="text-lg sm:text-xl font-bold mb-2 text-white" style={{ fontFamily: 'Heebo, sans-serif' }}>סקירת האלבום שלך</h3>
                                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, marginBottom: 16 }}>
                                    דפדפו בכל {MOCK_PAGES.length} העמודים. אשרו כשתהיו מוכנים ואנחנו נדפיס אותו.
                                </p>

                                <div className="space-y-3 mb-6">
                                    {[`${photos} תמונות מסודרות ב-${MOCK_PAGES.length} עמודים`, `ערכת נושא ${themeNameMap[theme] || theme} הוחלה`, 'כיתובים נוצרו באמצעות AI', 'ברכה דתית כלולה בעמוד 6'].map(item => (
                                ...
                                {!showApproveForm ? (
                                    <div className="space-y-3">
                                        ...
                                        <button className="btn-outline w-full justify-center" style={{ display: 'flex', width: '100%', fontSize: 14, borderColor: 'rgba(255,255,255,0.3)', color: 'rgba(255,255,255,0.6)' }}>
                                            <RotateCcw size={14} /> בקשת שינויים
                                        </button>
                                    </div>
                                ) : (
                                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                                        <textarea
                                            ...
                                            placeholder="הערות אחרונות? (אופציונלי) לדוגמה: 'נראה מושלם!' או 'בבקשה שנו את הגופן בעמוד 3'"
                                            rows={4}
                                            className="w-full rounded-lg px-4 py-3 text-sm outline-none resize-none mb-4 text-right"
                            ...
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl p-5 sm:p-8 text-center" style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)' }}>
                                ...
                                <h3 className="text-xl font-bold text-white mb-2" style={{ fontFamily: 'Heebo, sans-serif' }}>האלבום אושר!</h3>
                                <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>האלבום שלך נשלח כעת לדפוס. תקבלו עדכוני משלוח במייל.</p>
                            </motion.div>
                        )}

                        {/* פרטי חבילה */}
                        <div className="rounded-xl p-5 mt-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,0,0,0.15)' }}>
                            <p className="font-semibold mb-3" style={{ color: 'var(--primary)' }}>מה כלול</p>
                            {['אלבום כריכה קשה A4 (21 × 29.7 ס"מ)', 'נייר פורטרט 170 גרם', 'משלוח חינם', 'אחריות איכות 14 יום'].map(f => (
...
export default function PreviewPage() {
    return (
        <Suspense fallback={<div style={{ minHeight: '100vh', background: 'var(--dark)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div className="section-tag">טוען...</div></div>}>
            <PreviewPageContent />
        </Suspense>
    );
}
